...
policy_for() {
  case "$1" in
    ...
    RenderKit)            echo "AgentMCP" ;;  # concrete RenderExecuting backends (native SceneKit + usdrecord) injected into CLI/App-hosted MCP servers (#109); depends only on AgentMCP for the protocol
    ...
    App)                  echo "USDCore USDBridge ConversionKit ViewportKit EditingKit ValidationKit ScriptingKit EditorUI DicyaninDesignSystem MeshKit AgentMCP RenderKit SessionKit" ;;  # App hosts the in-app MCP editing session (specs/agent-live-editing.md); composition root, like CLI. SessionKit: names ViewState for the restore hand-off
    CLI)                  echo "USDCore USDBridge ConversionKit CaptureKit ValidationKit ScriptingKit EditingKit MeshKit AgentMCP RenderKit" ;;  # never EditorUI/DesignSystem; CaptureKit: `openusdz capture` pre-flight + plan (specs/capture-import.md)
